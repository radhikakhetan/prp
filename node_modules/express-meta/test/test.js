...
var fixturesDir = path.join(__dirname, 'fixtures'),
    expressDir = path.join(fixturesDir, 'node_modules','express'),
    krakenDir = path.join(fixturesDir, 'node_modules','kraken-js');

var express = require(expressDir),
    port = 8011;

var trackDir = process.cwd(),
    app = {
        basedir: fixturesDir,
...
describe('check meta - NON LIVE', function () {
    ...
    before(function (next) {
        ...
        var kraken = require(krakenDir);
        var httpsApp = express();

        httpsApp.use(kraken(app));
        wcServer = httpsApp.listen(port, function (err) {
            if (err) {
                console.error(err);
            }
    ...
    it('simple test', function (next) {
        inject('/meta', function (err, res) {
            ...
            assert.ok(JSON.stringify(model.middleware) !== '{}');
            assert.ok(model.processInfo.nodeEnv === 'test');
...
describe('check meta - LIVE Simulation', function () {
    ...
    before(function (next) {
        ...
        httpsApp.use(kraken(appProdWithCheckNext));
        wcServer = httpsApp.listen(port, function (err) {
            if (err) {
                console.error(err);
            }
    ...
    it('check production access accepted - request to exact host', function (next) {
        ...
        inject('/meta', function (err, res) {
            ...
            assert.ok(JSON.stringify(model.routes) !== '{}');
            assert.ok(JSON.stringify(model.middleware) !== '{}');
            assert.ok(model.processInfo.nodeEnv === 'production');
            next();
        }, params);
    });
    ...
    it('check production access accepted localhost', function (next) {
        ...
        inject('/meta', function (err, res) {
            ...
            assert.ok(JSON.stringify(model.routes) !== '{}');
            assert.ok(JSON.stringify(model.middleware) !== '{}');
            assert.ok(JSON.stringify(model.services) !== '{}');
            assert.ok(JSON.stringify(model.cdbsLayout) !== '{}');
            assert.ok(model.processInfo.nodeEnv === 'production');
            next();
        }, params);
    });
...
describe('check meta - NON-LIVE Simulation', function () {
    ...
    before(function (next) {
        this.timeout(10000);
        process.env.NODE_ENV = 'app_nextcheck';
        ...
        httpsApp.use(kraken(appWithCheckNext));
        wcServer = httpsApp.listen(port, function (err) {
            if (err) {
                console.error(err);
            }
