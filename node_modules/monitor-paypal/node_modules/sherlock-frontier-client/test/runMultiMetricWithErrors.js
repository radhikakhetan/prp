...
describe.skip('frontier mock server', function () {
    ...
    before(function (next) {
        frontierMockServer.listen(7300, function () {
            next();
        });
        frontierMockServer.setDisconnectCount(3);
    });

    describe('Midop Disconnect:', function (next) {

        it('many metrics sent with disconnect', function (next) {
    ...
    function postMetrics(metricIds, next) {
        ...
        fc.post(
            ...
            FrontierClient.PostReliability.BEST_EFFORT, function (err, result, latency) {
                console.log("BEST EFFORT CB", err, result);
                if (err) {
                    console.error("Post Sherlock metrics failed ", err);
                }
                if (next) {
                    return next(null);
                } else {
