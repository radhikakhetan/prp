...
module.exports = {
    ...
    connectionTest: function (options, data, callback) {
        if ((deployEnv.isDev() || deployEnv.isTest()) &&
            data &&
            options &&
            ...
            if (options.syncCheck) {
                var testResults = testConnections.test(data, options.connectionCheckTimeout);
                testConnections.print(testResults);

            } else {
                // Process only on the next event loop to avoid delay in socket onconnect
                // causing socket timeout
                setImmediate(function() {
                    testConnections.testAsync(data, options.connectionCheckTimeout,
